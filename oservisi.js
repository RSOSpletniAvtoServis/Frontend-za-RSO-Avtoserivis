  ...
  try {
    ...
    narocila.forEach(row => {
      html += `
        ...
        <td>`;
		if(row.ImaOceno == "false"){
			html += `<button onclick="podajOceno(${row.IDNarocilo})">Podaj Oceno</button>`;
		} else {
			html += `<button onclick="popraviOceno(${row.IDNarocilo})">Popravi Oceno</button>`;
		}
        html += `</td></tr>`;
...
function podajOceno(id) {
	const idtennant = document.getElementById('avtoser').value;
	localStorage.setItem('idn',id);
	localStorage.setItem('idt',idtennant);
	window.location.href = `podajoceno.html`;
...
function popraviOceno(id) {
	const idtennant = document.getElementById('avtoser').value;
	localStorage.setItem('idn',id);
	localStorage.setItem('idt',idtennant);
	window.location.href = `popravioceno.html`;
  // Option 1: redirect to edit page
  //window.location.href = `uredimodel.html?id=${id}`;
